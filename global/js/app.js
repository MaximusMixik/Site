(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".swiper-advisory")) new swiper_core_Swiper(".swiper-advisory", {
            ...
            scrollbar: {
                el: ".swiper-advisory .navigation__scrollbar",
                draggable: true
            },
            navigation: {
                prevEl: ".swiper-advisory .navigation__button--prev",
                nextEl: ".swiper-advisory .navigation__button--next"
            },
            breakpoints: {
                550: {
                    slidesPerView: 2.2
    ...
    da.init();
    function openItems() {
        const itemsList = document.querySelectorAll(".item-partner");
        ...
        function handleItemClick(e) {
            ...
            e.preventDefault();
            itemsList.forEach((element => {
                if (element !== this) element.classList.remove("open-item");
            }));
            this.classList.toggle("open-item");
        }
        function handleDocumentClick(e) {
            const clickedElement = e.target;
            if (!clickedElement.closest(".item-partner")) itemsList.forEach((element => {
                element.classList.remove("open-item");
            }));
        }
    }
    function advisoryAction() {
        const section = document.querySelector(".advisory");
        if (!section) return;
        section.addEventListener("click", (e => {
            const targetElement = e.target;
            if (targetElement.classList.contains("advisory-button") || targetElement.closest(".advisory-button")) if (window.innerWidth > 480) {
                const slide = targetElement.closest(".slide-advisory__wrapper");
                if (!slide) return;
                const popup = document.querySelector(".member-popup");
                const popupImage = popup.querySelector(".media-member__image img");
                const popupTitle = popup.querySelector(".media-member__title");
                const popupPosition = popup.querySelector(".media-member__position");
                const popupContent = popup.querySelector(".member-popup__content");
                const imageSrc = slide.querySelector(".slide-advisory__img img").getAttribute("src");
                const position = slide.querySelector(".slide-advisory__position").textContent;
                const name = slide.querySelector(".slide-advisory__text span").textContent;
                const content = slide.querySelector(".slide-advisory__text p").textContent;
                popupImage.setAttribute("src", imageSrc);
                popupTitle.textContent = name;
                popupPosition.textContent = position;
                popupContent.textContent = content;
                document.documentElement.classList.add("popup-open");
                const closeButton = popup.querySelector(".member-popup__close");
                ...
                const slide = targetElement.closest(".slide-advisory__wrapper");
                if (!slide) return;
                slide.classList.toggle("open-content");
            }
        }));
    }
    function casesSpollers() {
        const screenWidth = window.innerWidth;
        const items = document.querySelectorAll(".item-cases");
        if (!items.length) return;
        items.forEach((item => {
            const button = item.querySelector(".item-cases__button");
            function toggleContent() {
                item.classList.toggle("open-spoller");
            }
        ...
        function moveButton() {
            items.forEach((item => {
                const button = item.querySelector(".item-cases__button");
                const content = item.querySelector(".item-cases__content");
                if (screenWidth < 700) {
                    if (button && content && !content.contains(button)) content.after(button);
                } else {
                    const header = item.querySelector(".item-cases__header");
                    if (button && header && !header.contains(button)) header.appendChild(button);
                }
            }));
        }
        moveButton();
    ...
    function openSpoller() {
        ...
        if (detailsElement && detailsElement.tagName.toLowerCase() === "section") {
            const observer = new IntersectionObserver((entries => {
                entries.forEach((entry => {
                    if (entry.isIntersecting) setTimeout((() => {
                        const summaryElement = detailsElement.querySelector(".button-cases");
    ...
    window.onload = () => {
        ...
        advisoryAction();
        openSpoller();
        window.addEventListener("resize", casesSpollers);
    };
    window["FLS"] = false;
    menuInit();
    pageNavigation();
