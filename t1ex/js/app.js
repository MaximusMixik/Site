(() => {
    ...
    function initSliders() {
        if (document.querySelector(".swiper-media")) new swiper_core_Swiper(".swiper-media", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            grabCursor: true,
            ...
            spaceBetween: 24,
            navigation: {
                prevEl: ".media__button--prev",
                nextEl: ".media__button--next"
            },
            breakpoints: {
                500: {
                    slidesPerView: 2
        ...
        if (document.querySelector(".swiper-blog")) new swiper_core_Swiper(".swiper-blog", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            grabCursor: true,
    ...
    document.addEventListener("DOMContentLoaded", (function() {
            ...
            if (dropdownElements.length) {
                dropdownElements.forEach((el => {
                    el.addEventListener("click", (event => {
                        ...
                        if (el.classList.contains("dropdown-open")) el.classList.remove("dropdown-open"); else {
                            closeAllDropdowns();
        ...
        //! rate
                const rateContainers = document.querySelectorAll(".rate-exchange");
        ...
        function handleRateClick(e) {
            ...
            const rateInput = targetElement.querySelector(".rate-exchange__input");
            const rateDescription = targetElement.querySelector(".rate-exchange__description");
            const rateImage = targetElement.querySelector(" .rate-exchange__safe img");
            const isLocked = targetElement.dataset.locked === "true";
            rateInput.textContent = isLocked ? "lock" : "unlock";
            targetElement.dataset.locked = !isLocked;
        ...
        if (selectDropdown.length) selectDropdown.forEach((el => {
            ...
            class CryptList {
                sortList(findText) {
                    const crypt = el.querySelectorAll(".option__item");
                    crypt.forEach((crypt => {
                        const name = crypt.textContent.toLowerCase();
                        if (name.trim().startsWith(findText.toLowerCase())) crypt.style.display = "flex"; else crypt.style.display = "none";
                    }));
                }
            }
            class CurrencySelector {
                constructor() {
                    this.optionItems = el.querySelectorAll(".option__item");
                    this.selectLabel = el.querySelector(".select__button");
                    this.selectWrapper = el.querySelector(".select__wrapper");
                    this.select = el;
                    this.attachEvents();
                }
                attachEvents() {
            ...
            selectBtn.addEventListener("click", (() => el.classList.toggle("active")));
            closeBtn.addEventListener("click", (() => el.classList.remove("active")));
