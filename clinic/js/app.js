    ...
    (() => {
        ...
        function initSliders() {
            if (document.querySelector(".main__slider")) {
                const mainSlider = new swiper_core_Swiper(".main__slider", {
                    ...
                    pagination: {
                        el: ".main-navigation__body",
                        clickable: true,
                        renderBullet: function(index, className) {
                            return `<div data-slide="${index}" class="${className}"> ${index < 10 ? "0" + (index + 1) : index + 1}</div><span > </span>`;
                        }
                    },
                    navigation: {
                        prevEl: ".swiper-button-prev",
                ...
                document.addEventListener("click", (function(e) {
                    const targetElement = e.target;
                    if (targetElement.classList.contains("swiper-pagination-bullet") && targetElement.closest(".main-navigation ")) {
...
***************************************************************************** */
        ...
        da.init();
        const beforeAfter = document.querySelector(".befor-after");
        if (beforeAfter) {
            const beforeAfterArrow = document.querySelector(".befor-after__line");
            const afterItem = document.querySelector(".befor-after__item_after");
            beforeAfter.addEventListener("mouseover", (function(e) {
                const targetElement = e.target;
                if (!targetElement.classList.contains("befor-after__line")) if (targetElement.closest(".befor-after__item_before")) {
                    beforeAfterArrow.classList.add("befor-after__line_right");
                    beforeAfterArrow.classList.remove("befor-after__line_left");
                } else {
                    beforeAfterArrow.classList.remove("befor-after__line_right");
                    beforeAfterArrow.classList.add("befor-after__line_left");
                }
            }));
            ...
            beforeAfterArrow.addEventListener("mousedown", (function(e) {
                ...
                function beforeAfterArrowMove(e) {
                    ...
                    if (posLeft <= beforeAfterSizes.width && posLeft > 0) {
                        ...
                        beforeAfterArrow.style.cssText = `left: ${way}%`;
                        afterItem.style.cssText = `width: ${100 - way}%`;
                    } else if (posLeft >= beforeAfterSizes.width) {
                        beforeAfterArrow.style.cssText = `left: 100%  `;
        ...
        const doctorsList = document.querySelector(".doctor");
        const doctorPhoto = document.querySelector(".body-doctors__photo");
        const doctorSkills = document.querySelector(".doctor-skills");
        const doctorStatistics = document.querySelector(".doctor-info__statistics");
        const doctorGallery = document.querySelector(".doctors__gallery");
        const doctorListActive = "active";
        function initDoctors(data) {
            let doctorsListElements = ``;
            data.doctors.forEach((doctor => {
                const isActive = doctor.isActive;
                if (isActive) buildDoctor(doctor);
                doctorsListElements += `<a data-id="${doctor.id}" href="#" class="doctor__item ${doctor.isActive ? doctorListActive : null}">\n\t\t\t\t\t\t\t\t<div class="doctor__img-ibg">\n                <img src="img/${doctor.media.avatar}" alt="${doctor.name}"></div>\n\t\t\t\t\t\t\t\t<div class="doctor__info">\n\t\t\t\t\t\t\t\t\t<div class="doctor__name">${doctor.name}</div>\n\t\t\t\t\t\t\t\t\t<div class="doctor__text text">\n\t\t\t\t\t\t\t\t\t${doctor.position}\n\t\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t</a>`;
            }));
            doctorsList.innerHTML = doctorsListElements;
            document.addEventListener("click", doctorsListActions);
            function doctorsListActions(e) {
                const targetElement = e.target;
                if (targetElement.closest(".doctor__item")) {
                    const currentItem = targetElement.closest(".doctor__item");
                    const doctorId = +currentItem.dataset.id;
                    const activeListItem = document.querySelector(".doctor__item.active");
        ...
        function buildDoctor(activeDoctor) {
            ...
            const skills = activeDoctor.skills;
            const statistics = activeDoctor.statistics;
            const gallerySmallItems = activeDoctor.media.gallery.small;
            const galleryBigItems = activeDoctor.media.gallery.big;
            activeDoctor.media.gallery;
            let skillItem = ``;
            let statisticsItem = ``;
            let galleryItem = ``;
            mainPhoto ? doctorPhoto.innerHTML = `<img src="img/${mainPhoto}" alt="Stephen Mcmiller">` : null;
            for (const item in skills) skillItem += `\n      \t\t<div class="doctor-skills__item">\n  \t\t\t\t\t\t\t\t<div  style="width: ${skills[item]}%;" class="doctor-skills__body">\n  \t\t\t\t\t\t\t\t\t<div class="doctor-skills__label">${item}</div>\n  \t\t\t\t\t\t\t\t\t<div class="doctor-skills__value">${skills[item]}%</div>\n  \t\t\t\t\t\t\t\t</div>\n                  \t<div style="width: ${skills[item]}%;" class="doctor-skills__line"></div>\n  \t\t\t\t\t\t\t</div>\n      `;
            doctorSkills.innerHTML = skillItem;
            for (const item in statistics) statisticsItem += `\n      \t<div class="doctor-statistics__item">\n      \t\t\t\t\t\t<div class="doctor-statistics__value">${statistics[item]}</div>\n      \t\t\t\t\t\t<div class="doctor-statistics__label">${item}</div>\n      \t\t\t\t\t</div>\n      `;
            doctorStatistics.innerHTML = statisticsItem;
            gallerySmallItems.forEach(((gallerySmallItems, index) => {
                galleryItem += `\n            \t\t\t\t\t\t<a href="img/${gallerySmallItems}" class="gallery-footer__item"><img\n        \t\t\t\t\tsrc="img/${galleryBigItems[index]}" alt="gallery item"></a>`;
            }));
            doctorGallery.innerHTML = galleryItem;
        ...
        if (gallery && !isMobile.any()) window.addEventListener("scroll", (function(e) {
            ...
            const gRow = document.querySelectorAll(".gallery__line");
            const wHeight = window.innerHeight;
            gRow.forEach(((gRowItem, index) => {
                const gItems = gRowItem.querySelectorAll(".item-gallery");
                let dRowWidth = 0;
                ...
                if (gWay > 0) {
                    ...
                    } else gRowItem.style.cssText = `transform: translateX(0);`;
                }
            }));
        }));
        window["FLS"] = true;
