(() => {
    ...
    (() => {
        ...
        document.addEventListener("click", (function(e) {
            const targetElement = e.target;
            if (targetElement.closest(".header-search__icon")) {
                document.documentElement.classList.add("search-open");
                e.preventDefault();
            } else if (!targetElement.closest(".header-search")) document.documentElement.classList.remove("search-open");
            if (targetElement.closest(".account-form__link-pass")) {
                const buttonChangePassword = targetElement.closest(".account-form__link-pass");
                changePassword(buttonChangePassword);
            }
            if (targetElement.closest(".checkbox") && targetElement.closest(".filter__body") && targetElement.tagName !== "INPUT") filterClearShow(targetElement);
            if (targetElement.closest(".filter-header__clear")) {
                const clearButton = targetElement.closest(".filter-header__clear");
                filterBodyClear(clearButton);
            }
        }));
        function changePassword(targetElement) {
            const accountBody = targetElement.closest(".account-form");
            accountBody.classList.add("open-pass");
        }
        function filterClearShow(targetElement) {
            const filterBody = targetElement.closest(".filter__body");
            const filterClear = filterBody.querySelector(".filter-header__clear");
            setTimeout((() => {
                const checkedElements = filterBody.querySelectorAll(".checkbox__input:checked");
                checkedElements.length ? filterClear.classList.add("_active") : filterClear.remove("_active");
            }), 0);
        }
        function filterBodyClear(clearButton) {
            const filterBody = clearButton.closest(".filter__body");
            const checkedElements = filterBody.querySelectorAll(".checkbox__input:checked");
