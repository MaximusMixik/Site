 ...
 */
    ...
    (() => {
        ...
        function initSliders() {
            if (document.querySelector(".why-me-swiper")) new swiper_core_Swiper(".why-me-swiper", {
...
***************************************************************************** */
        ...
        function script_formFieldsInit() {
        ...
        function initProgressBar() {
            ...
            window.addEventListener("scroll", (function() {
                ...
                const progressBar = document.querySelector(".progress-bar");
                progressBar.querySelector(".progress-bar__line");
                const progressCircle = progressBar.querySelector(".progress-bar__progress");
                ...
                if (scrolled >= 98) progressBar.classList.add("full"); else progressBar.classList.remove("full");
            }));
        }
        //! V2
                function closeBurgerAndSmooth() {
        ...
        function checkAndScrollToSavedElement() {
        ...
        function script_Zoom(imageId, zoomLevel) {
            const image = document.getElementById(imageId);
            if (!image) return;
            const loupe = document.createElement("div");
            loupe.classList.add("magnifier-glass");
            image.parentElement.insertBefore(loupe, image);
            const updateLoupeStyle = () => {
                loupe.style.backgroundImage = `url('${image.src}')`;
                loupe.style.backgroundRepeat = "no-repeat";
                loupe.style.backgroundSize = `${image.width * zoomLevel}px ${image.height * zoomLevel}px`;
            };
            updateLoupeStyle();
            const width = loupe.offsetWidth / 2;
            const height = loupe.offsetHeight / 2;
            const back = 2;
            function moveLoupe(e) {
                e.preventDefault();
                const {x, y} = getMousePosition(e);
                const clampedX = Math.max(Math.min(x, image.width - width / zoomLevel), width / zoomLevel);
                const clampedY = Math.max(Math.min(y, image.height - height / zoomLevel), height / zoomLevel);
                loupe.style.left = `${clampedX - width}px`;
                loupe.style.top = `${clampedY - height}px`;
                loupe.style.backgroundPosition = `-${clampedX * zoomLevel - width + back}px -${clampedY * zoomLevel - height + back}px`;
            }
            ...
            const eventTarget = image.parentElement;
            eventTarget.addEventListener("mousemove", moveLoupe);
            eventTarget.addEventListener("touchmove", moveLoupe);
            window.addEventListener("resize", updateLoupeStyle);
        }
        window.onload = () => {
            initProgressBar();
            script_formFieldsInit();
            closeBurgerAndSmooth();
            checkAndScrollToSavedElement();
            if (window.innerWidth > 768) script_Zoom("dinant", 2);
        };
        window["FLS"] = false;
        isWebp();
        menuInit();
